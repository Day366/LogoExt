...
namespace LogoExt
{
    class Query
    {
        ...
        {
            DataTable dt = QueryDB("SELECT format(Invoice.DATE_, 'dd.MM.yyyy') as 'Tarih', Invoice.FICHENO as 'Fiş No.', CONVERT(varchar(10), Invoice.TRCODE) as 'Türü', Clcard.DEFINITION_ as 'Müşteri Ünvanı', FORMAT(Invoice.NETTOTAL, '####,##0.00') as 'Turar', FORMAT(Invoice.REPORTNET, '####,##0.00') as 'Dövizli Turar', CASE WHEN Invoice.EINVOICE = 1 THEN (CASE WHEN Invoice.Estatus = 0 THEN 'GİB''e Gönderilecek' WHEN Invoice.Estatus = 10 THEN 'Alıcıda İşlendi - Başarıyla Tamamlandı' WHEN Invoice.Estatus = 12 THEN 'Kabul Edildi' WHEN Invoice.Estatus = 13 THEN 'Reddedildi' WHEN Invoice.Estatus = 22 THEN 'Sunucuya Gönderildi' ELSE '********HATALI********' END) WHEN Invoice.EINVOICE = 2 THEN (CASE WHEN Invoice.Estatus = 0 THEN 'E-arşiv Fatura Oluşturulacak' WHEN Invoice.Estatus = 2 THEN 'Rapor Dosyasına Yazıldı' ELSE '********HATALI********' END) END as 'E-Fatura Statüsü' FROM LG_0" + invoiceForm.GetFirmCode(0) + "_01_INVOICE Invoice, LG_0" + invoiceForm.GetFirmCode(0) + "_CLCARD Clcard WHERE Invoice.TRCODE in (2, 3, 7, 8, 9, 10, 14) and Clcard.LOGICALREF = Invoice.CLIENTREF ORDER BY Invoice.DATE_, Invoice.FICHENO");
            return dt;
        }
