...
namespace LogoExt
{
    class Query
    {
        ...
        {
            DataTable dt = QueryDB("SELECT format(Invoice.DATE_, 'dd.MM.yyyy') as 'Tarih', Invoice.FICHENO as 'Fiş No.', CONVERT(varchar(10), Invoice.TRCODE) as 'Türü', Clcard.DEFINITION_ as 'Müşteri Ünvanı', FORMAT(Invoice.NETTOTAL, '####,##0.00') as 'Turar', FORMAT(Invoice.REPORTNET, '####,##0.00') as 'Dövizli Turar', CASE WHEN Invoice.EINVOICE = 1 THEN (CASE WHEN Invoice.Estatus = 0 THEN 'GİB''e Gönderilecek' WHEN Invoice.Estatus = 4 THEN 'GİB''e Gönderildi' WHEN Invoice.Estatus = 6 THEN 'GİB''e İşlendi. Alıcıya İletilecek' WHEN Invoice.Estatus = 10 THEN 'Alıcıda İşlendi - Başarıyla Tamamlandı' WHEN Invoice.Estatus = 12 THEN 'Kabul Edildi' WHEN Invoice.Estatus = 13 THEN 'Reddedildi' WHEN Invoice.Estatus = 22 THEN 'Sunucuya Gönderildi' ELSE '********HATALI********' END) WHEN Invoice.EINVOICE = 2 THEN (CASE WHEN Invoice.Estatus = 0 THEN 'E-arşiv Fatura Oluşturulacak' WHEN Invoice.Estatus = 2 THEN 'Rapor Dosyasına Yazıldı' ELSE '********HATALI********' END) END as 'E-Fatura Statüsü' FROM LG_0" + invoiceForm.GetFirmCode(0) + "_01_INVOICE Invoice, LG_0" + invoiceForm.GetFirmCode(0) + "_CLCARD Clcard WHERE Invoice.TRCODE in (2, 3, 7, 8, 9, 10, 14) and Clcard.LOGICALREF = Invoice.CLIENTREF ORDER BY Invoice.DATE_, Invoice.FICHENO");
            return dt;
        }

        //Satış faturası detayları
        public DataTable QueryInvoiceDetails(InvoiceForm invoiceForm, string ficheno)
        {
            DataTable dt = QueryDB("SELECT Items.CODE as 'Kodu', Items.NAME as 'Açıklama', Stline.AMOUNT as 'Miktar', (Select CODE FROM LG_0" + invoiceForm.GetFirmCode(0) + "_UNITSETF Unitsetf WHERE Unitsetf.LOGICALREF = Stline.USREF) as 'Birim', Stline.PRICE as 'Birim Fiyatı', CASE WHEN Stline.PRCURR = 0 and Stline.REPORTRATE != 0 THEN Round(Stline.PRICE / Stline.REPORTRATE, 5) WHEN Stline.PRCURR = 20 THEN ROUND(Stline.PRPRICE, 2) WHEN Stline.PRCURR = 11 THEN ROUND(Stline.PRPRICE, 2) WHEN Stline.PRCURR = 1 THEN ROUND(Stline.PRPRICE, 2) END as 'Dövizli Birim Fiyatı', CASE WHEN (Stline.PRCURR = 0 and Stline.REPORTRATE != 0) or Stline.PRCURR = 20 THEN 'EUR' WHEN Stline.PRCURR = 11 THEN 'CHF' WHEN Stline.PRCURR = 1 THEN 'USD' ELSE 'TL' END as 'Döviz Türü', Stline.LINEEXP as 'Açıklamalar', Stline.SPECODE as 'Hareket Özel Kodu', (SELECT Emuhacc.DEFINITION_ FROM LG_0" + invoiceForm.GetFirmCode(0) + "_EMUHACC Emuhacc WHERE Emuhacc.LOGICALREF = Stline.VATACCREF) as 'KDV', Stline.TOTAL as 'Tutarı', Round(Stline.TOTAL / Stline.REPORTRATE, 2) as 'Dövizli Tutar', Round(Stline.TOTAL / Stline.REPORTRATE * 0.18, 2) as 'Dövizli KDV Tutarı' FROM LG_0" + invoiceForm.GetFirmCode(0) + "_01_INVOICE Invoice, LG_0" + invoiceForm.GetFirmCode(0) + "_01_STLINE Stline, LG_0" + invoiceForm.GetFirmCode(0) + "_ITEMS Items, LG_0" + invoiceForm.GetFirmCode(0) + "_UNITSETF Unitsetf WHERE Invoice.LOGICALREF = Stline.INVOICEREF and Items.LOGICALREF = Stline.STOCKREF and Unitsetf.LOGICALREF = Stline.USREF and Invoice.FICHENO = '" + ficheno + "'");
            return dt;
        }
