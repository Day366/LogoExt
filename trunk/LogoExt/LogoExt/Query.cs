...
namespace LogoExt
{
    class Query
    {
        ...
        {
            DataTable dt = QueryDB("SELECT format(Stline.DATE_, 'dd.MM.yyyy') AS 'Tarih', ISNULL((SELECT Stfiche.FICHENO FROM LG_0" + itemsForm.GetFirmCode(0) + "_01_STFICHE Stfiche WHERE Stfiche.LOGICALREF = Stline.STFICHEREF), '') as 'Fiş No.', ISNULL((SELECT Invoice.DOCODE FROM LG_0" + itemsForm.GetFirmCode(0) + "_01_INVOICE Invoice where Invoice.LOGICALREF = Stline.INVOICEREF), '') as 'Fiş Belge No.', CASE WHEN Stline.CANCELLED = 0 and Stline.LINETYPE <> 4 THEN (CASE WHEN Stline.TRCODE = 51 THEN 'Sayım Eksiği Fişi' WHEN Stline.TRCODE = 50 THEN 'Sayım Fazlası Fişi' WHEN Stline.TRCODE = 26 THEN 'Mustahsil İrsaliyesi' WHEN Stline.TRCODE = 25 THEN 'Ambar Fişi' WHEN Stline.TRCODE = 14 THEN 'Devir Fişi' WHEN Stline.TRCODE = 13 THEN 'Üretimden Giriş Fişi' WHEN Stline.TRCODE = 12 THEN 'Sarf Fişi' WHEN Stline.TRCODE = 11 THEN 'Fire Fişi' WHEN Stline.TRCODE = 10 THEN 'Konsinye Giriş İade İrsaliyesi' WHEN Stline.TRCODE = 9 THEN 'Konsinye Çıkış İrsaliyesi' WHEN Stline.TRCODE = 8 THEN 'Toptan Şatış İrsaliyesi' WHEN Stline.TRCODE = 7 THEN 'Perakende Şatış İrsaliyesi' WHEN Stline.TRCODE = 6 THEN 'Alım İade İrsaliyesi' WHEN Stline.TRCODE = 5 THEN 'Konsinye Giriş İrsaliyesi' WHEN Stline.TRCODE = 4 THEN 'Konsinye Çıkış İade İrsaliyesi' WHEN Stline.TRCODE = 3 THEN 'Toptan Şatış İade İrsaliyesi' WHEN Stline.TRCODE = 2 THEN 'Perakende Şatış İade İrsaliyesi' WHEN Stline.TRCODE = 1 THEN 'Satınalma İrsaliyesi' ELSE '********HATALI********' END) END AS 'Tür', ISNULL((SELECT Clcard.DEFINITION_ FROM LG_0" + itemsForm.GetFirmCode(0) + "_CLCARD Clcard WHERE Clcard.LOGICALREF = Stline.CLIENTREF), '') AS 'Cari Hesap Ünvanı', Stline.AMOUNT AS 'Miktar', Stline.LINEEXP AS 'Satır Açıklaması', FORMAT(Stline.PRICE, '####,##0.00')as 'Birim Fiyat', FORMAT(Stline.TOTAL, '####,##0.00') as 'Tutar' FROM LG_0" + itemsForm.GetFirmCode(0) + "_01_STLINE Stline, LG_0" + itemsForm.GetFirmCode(0) + "_ITEMS Items WHERE Stline.STOCKREF = Items.LOGICALREF and Items.CODE = '" + selectedItem + "' and not ((Stline.TRCODE = 13 or Stline.TRCODE = 14) and Stline.SOURCELINK <> 0) ORDER BY Stline.DATE_");
            return dt;
        }
